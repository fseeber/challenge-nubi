...
class CurrencyControllerTest {
    ...
    void testConvertCurrencyApiTimeout() throws Exception {
        // Mock del servicio para simular un timeout
        Mockito.when(currentService.convertCurrency("USD", "EUR", 100.0))
               .thenThrow(new ApiTimeoutException("El servicio de conversión no respondió a tiempo."));

        // Realiza la solicitud y verifica el manejo del timeout
        mockMvc.perform(post("/convert")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"baseCurrency\":\"USD\",\"targetCurrency\":\"EUR\",\"amount\":100}"))
                .andDo(print())
                .andExpect(status().isGatewayTimeout())
                .andExpect(content().string("El servicio de conversión no respondió a tiempo."));
    }
    ...
    void testConvertCurrencyInternalError() throws Exception {
        ...
        mockMvc.perform(post("/convert")
                ...
                .andExpect(content().string("Error en la conversión: Error inesperado."));
    }
}
